...
/// Looks up the shop's UPI ID for the "pay by UPI" option — not stored
/// in session (which only keeps id/name/code), so this fetches the full
/// shop record by code once, on demand.
final _shopDetailsProvider = FutureProvider.autoDispose<Map<String, dynamic>?>((ref) async {
  final session = ref.watch(sessionProvider);
  if (session.shopCode == null) return null;
  final api = ref.watch(_apiServiceProvider);
  return api.getShopByCode(session.shopCode!);
});
...
class _CheckoutScreenState extends ConsumerState<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final shopDetailsAsync = ref.watch(_shopDetailsProvider);
            ...
            children: [
                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            const SizedBox(height: AppSpacing.md),
                            QrImageView(data: upiUri, size: 160, backgroundColor: Colors.white),
                            const SizedBox(height: AppSpacing.sm),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: (checkoutState.isLoading || lines.isEmpty) ? null : _placeOrder,
